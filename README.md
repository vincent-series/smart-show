...
下面是一些公共方法
<pre><code>
public interface INormalDialogCreator<C> {
    //Dialog显示时窗口以外区域是否变暗
    C darkAroundWhenShow(boolean dim);
    //设置窗口背景
    C windowBackground(@DrawableRes int bgRes);
    //按back键是否可以取消
    C cancelable(boolean b);
    //触碰Dialog窗口外的区域是否取消掉Dialog
    C cancelableOnTouchOutside(boolean b);
