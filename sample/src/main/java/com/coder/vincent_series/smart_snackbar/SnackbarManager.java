    ...
    public void show(int duration, Callback callback) {
        synchronized (lock) {
            ...
            if (currentSnackbar != null
                    && cancelSnackbarLocked(currentSnackbar, SnackBar.BaseCallback.DISMISS_EVENT_CONSECUTIVE)) {
    ...
    void handleTimeout(@NonNull SnackbarRecord record) {
        synchronized (lock) {
            if (currentSnackbar == record || nextSnackbar == record) {
                cancelSnackbarLocked(record, SnackBar.BaseCallback.DISMISS_EVENT_TIMEOUT);
            }
        }
    }
}
